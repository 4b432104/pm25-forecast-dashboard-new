...
def main():
    ...
    # 2. 擷取 17 項即時數據與除錯 Log
    live_features_list = [0.0] * 17
    debug_logs = []

    with st.spinner(f"📡 正在擷取霧峰即時監測與車流數據 ({traffic_range_str})..."):
        try:
            live_features, debug_logs = application.fetch_wufeng_live_features()
    ...
    # ---------------------------------------------------------
    # 🛠️ 工作日記 / 車流與 API 擷取診斷 log 區塊
    # ---------------------------------------------------------
    with st.expander("🔍 點此展開【資料擷取工作日記 / 高公局車流診斷紀錄】", expanded=False):
        st.caption("呈現系統最近一次嘗試獲取高公局車流 CSV 與環測 API 的完整調試歷程：")
        for log_item in debug_logs:
            if "❌" in log_item or "⚠️" in log_item or "💥" in log_item:
                st.error(log_item)
            elif "✅" in log_item or "🎯" in log_item or "📄" in log_item:
                st.success(log_item)
            else:
                st.text(log_item)
